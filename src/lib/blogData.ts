import RoofingPost1 from "@/content/posts/Roofing/post-1";
import PlumbingPost1 from "@/content/posts/Plumbing/post-1";
import HvacPost1 from "@/content/posts/Hvac/post-1";

export const blogPosts = [
  {
    id: 1,
    slug: "how-to-get-roofing-leads-using-facebook-remarketing-ads",
    category: "roofing",
    image: "/blog/post-1.jpg",
    title: "How to Get Roofing Leads Using Facebook Remarketing Ads",
    date: "June 25, 2025",
    name: "John Doe",
    description:
      "This guide shows you how to use Facebook remarketing to bring those visitors back and turn them into paying roofing customers. Learn how to set up Meta Pixel, build smart audiences, create high-converting ads, and track real ROI. Includes case studies, advanced targeting tips, and ad sequences tailored to roofers. Perfect for any roofing business w...",
    Content: RoofingPost1,
  },
  {
    id: 2,
    slug: "how-to-get-roofing-leads-using-real-estate-referrals",
    category: "roofing",
    image: "/blog/post-2.jpg",
    title: "How to Get Roofing Leads Using Real Estate Referrals",
    date: "June 20, 2025",
    name: "Jane Smith",
    description:
      "This guide shows you how to train agents to send you steady, high-quality roofing leads using pre-listing inspections. You'll learn how to create, market, and deliver an agent training program that benefits everyone—agents, sellers, and your roofing business. Stop relying on outdated roofing marketing strategies and start building a referral machin...",
    Content: RoofingPost1,
  },
  {
    id: 3,
    slug: "how-to-get-local-roofing-leads-using-nextdoor",
    category: "roofing",
    image: "/blog/post-3.jpg",
    title: "How to Get Local Roofing Leads Using Nextdoor",
    date: "June 15, 2025",
    name: "Michael Chen",
    description:
      "In this guide, you'll learn how to create a high-converting business profile, post helpful content that builds trust, and tap into storm-driven demand. We'll show you how to turn happy customers into lead magnets, use hyper-local paid ads, and track everything to grow your roofing business fast—without spending big on third-party lead providers.",
    Content: RoofingPost1,
  },
  {
    id: 4,
    slug: "3-easy-ways-to-get-plumbing-leads",
    category: "plumbing",
    image: "/blog/plumbing-1.jpg",
    title: "3 Easy Ways to Get Plumbing Leads: A Universal Guide",
    date: "June 20, 2025",
    name: "Alex Gambashidze",
    description:
      "Learn three proven strategies to boost plumbing leads with minimal cost. From Google reviews to pay-per-call marketing and Facebook, this guide helps your plumbing business stand out",
    Content: PlumbingPost1,
  },
  {
    id: 5,
    slug: "hvac-leads-google",
    category: "hvac",
    image: "/blog/hvac-1.jpg",
    title: "How To Quickly Grow Your HVAC Business: 4 Proven Tactics",
    date: "June 20, 2025",
    name: "Sarah Lee",
    description:
      "Learn four proven tactics to quickly grow your HVAC business. This guide covers Google reviews, referral networks, paid ads, and pay-per-call strategies to boost lead flow and drive growth.",
    Content: HvacPost1,
  },
];
